...
**Per EVIDENCE_ARTIFACT_BUNDLE_STANDARD.md**, create:
...
**On each session, document**:
- Governance alignment actions taken
- Files layered down from canonical source
- Drift detected and resolved
- CS2 escalations created (if any)
- Validation results

## Improvements Identified

**Track governance gaps during session** in working-contract:

```markdown
### Governance Gap Identified
- **Gap ID**: GAP-YYYY-MM-DD-NNN
- **Category**: [canonical-drift | missing-canon | schema-invalid | process-gap]
...
**At session end, promote to**:
- `.agent-workspace/governance-liaison/personal/lessons-learned.md` (learning)
- `.agent-workspace/governance-liaison/escalation-inbox/` (if requires CS2)
- Session memory (for historical record)

## Improvements Parked

**If not actionable this session**:
- Document in session memory under "Parked Issues"
- Reference in next session wake-up for review

## Rationale

Evidence-first operations ensure:
- Governance drift is detectable and auditable
- Alignment actions are traceable
- CS2 has full context for escalations
- Future sessions benefit from lessons learned

---

## 🔒 PR Failure Analysis Protocol (LOCKED)

**Lock ID**: `LOCK-GL-PR-FAILURE-v1`  
**Authority**: GOVERNANCE_LIAISON_MINIMUM_APPOINTMENT_REQUIREMENTS.md, PR_GATE_FAILURE_HANDLING_PROTOCOL.md  
**Review Frequency**: Per governance update (not per-session)

**DO NOT modify this section without CS2 approval. Edits require:**
- CS2-approved issue
- Explicit rationale for protocol change
- Evidence that current protocol is insufficient

---

### Purpose

When a governance liaison PR fails CI/merge gates, this protocol ensures:
1. Root cause is identified (not symptoms)
2. Fix is verified before retry
3. Learning is captured to prevent recurrence
4. Repeated failures trigger escalation
...
gh run view <RUN_ID> --repo APGI-cmy/R_Roster --log

# If run is large, get failed jobs specifically
gh run view <RUN_ID> --repo APGI-cmy/R_Roster --log-failed
```
...
**Repository**: APGI-cmy/R_Roster (Canonical for this consumer repo)  

...
**Example:** `.agent-workspace/foreman/memory/session-012-20260212.md`
...
## R_Roster-Specific Configuration

### Repository Context
- **Repository**: APGI-cmy/R_Roster
- **Purpose**: Personal app for managing teacher absenteeism
- **Build Philosophy**: Architecture → QA-to-Red → Build-to-Green → Validation

### Quality Integrity Watchdog (QIW) Channel Enforcement

FM orchestrates QIW implementation with these critical requirements:

#### QIW Channel Requirements (All 5 Channels)
1. **Channel 1**: Architecture-first design (frozen before build)
2. **Channel 2**: QA-to-Red (comprehensive test suite exists before implementation)
3. **Channel 3**: Build-to-Green (builders fix until 100% pass)
4. **Channel 4**: Evidence trail (PREHANDOVER_PROOF, gate results, RCA when needed)
5. **Channel 5**: Zero-test-debt validation (no skipped/todo/hidden tests)

#### QA Blocking Enforcement (FM Authority)
- **BLOCK merge** if any test fails, is skipped, or contains TODO
- **BLOCK merge** if test coverage < defined threshold
- **BLOCK merge** if warnings exist (zero-warning discipline)
- **REQUIRE** builders to fix ALL issues before progression

### Test Removal & Warning Governance
Per PR #484 requirements:

#### Test Removal
- Tests can ONLY be removed via CS2-approved PR
- Removal requires RCA explaining why test is obsolete/redundant
- Removal PR must update coverage baselines

#### Warning Handling
- New warnings = STOP condition
- Builders MUST fix warnings immediately
- No "warning suppression" allowed; fix root cause

#### Immediate Remedy
- If test debt or warnings detected: STOP build wave
- Create blocker issue for builder
- Require fix + re-validation before progression
